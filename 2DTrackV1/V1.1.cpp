//V1.2
//fixed issue of cell being considered cell of previous frame
...
int main(int argc, char** argv){
    ...
    while(1)
    {
        ...
        if(contours.size() > 0)
          sort(contours.begin(),contours.end(),compCont);
		  	...
		  	vector<bool> present(cells.size(),false),  newPresent(cells.size(),false);
        ...
        for(int i = 0; i < contours.size() && contourArea(contours[i]) > 5; i++)
         {
            ...
            circle(processed, center,40, Scalar(0,0,255));//outer ring
            int possibleIdx = -1;
              ...
              if(cells[j].size() == 0){
	                ...
	                cells[j].push_back(center);
						  ...
						  //	cout << cells[12].size() << " " << cells[12].back().x << endl;
							...
							//cout << cells[j][cells[j].size()-2].x << "," << cells[j][cells[j].size()-2].y << endl;
            ...
            if(possibleIdx != -1){
              ...
              present[possibleIdx] = true;
				 ...
				 for(int j = 0; j < present.size(); j++){
					 if(!present[j] && cells[j].size() != 0 && cells[j].back().x != -1)
						 cells[j].push_back(end);
				 }
    ...
    for(int i = 0; i < cells.size(); i++){
      ...
      for(int j = 0; j < cells[i].size(); j++){
        if(cells[i][j].x == -1)
          break;
				fout << cells[i][j].x << "," << cells[i][j].y << endl;
      }
    }
