...
//What's not done: edge cases, remove extra points
...
//OTHER VALUES TO BE CHANGED: conditional satement to determine if it is a cell or not, minDiff
...
vector<int> findPeaks(vector<float> rad, int length, bool test = 0){//can save "power value" to rule out bad ones (same for bad radii)
  vector<int> peakIndex(length);
  const int width = 7;//width of sample to take the mean of to look at
  if(length < width)
    return peakIndex;
  float minDiff = 2;//min difference between mean of the n values and the center value to be considered a peak
  float val = 0;//computed mean
  int multiplier[width] = {3,4,1,0,1,4,3};//we can use different multipliers for small and large peak detection
  int num = 0; //sum of multipliers, number to divide value by
  int count = 0;
  ...
  for(int i = 0; i < length - 1 - width; i++){
    for(int j = 0; j < width; j++){
      val += multiplier[j] * rad[j+i];
    }
    val /= num;
    if(test)
      cout << rad[i+width/2] - val << endl;
    ...
    for(int thresh = 250; thresh >= 50; thresh -= 3){
      ...
      for(int i = 0; i < contours.size() && i < 7; i++)
      {
         ...
         if(approx.size() > 8 && radius*radius*PI/contourArea(contours[i]) < 1.3 && contourArea(contours[i]) > minArea){//airy pattern: inner ring, 15% margin or error, previous values: 8, 1.3
          ...
          if(center.x > 868.4 && center.x < 868.5)
            peakIndices = findPeaks(rad,maxRadius,1);
          else
            peakIndices = findPeaks(rad,maxRadius,0);
          int peakNum = -1;
          while(peakIndices[++peakNum] != -1);
          cout << "(" << center.x << "," << center.y << "):" << peakIndices[peakNum - 1] << "       Num Peaks:" << peakNum << endl;//ANSWERS
          circle(newIm, center,peakIndices[peakNum - 1], Scalar(125),2);
         }
      }
    }
    namedWindow("test",0);
